...
const ManageLands = () => {
  ...
  useEffect(() => {
    if (terrainId) {
      loadTerrainAndGardens();
    }
  }, [terrainId]);


      if (terrainError) {
        console.error('Error loading terrain:', terrainError);
        toast({
          title: "Erreur",
  ...
  const canAddMoreGardens = terrain ? gardens.length < terrain.number_of_gardens : false;
    ...
    <div className="min-h-screen pt-16 pb-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="py-6">
          ...
          <div className="mb-6">
            <h1 className="text-3xl font-bold">{terrain.name}</h1>
            <p className="text-muted-foreground">{terrain.address}</p>
            <p className="text-sm text-muted-foreground">
              Référence cadastrale: {terrain.cadastral_reference}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              <Badge variant="secondary">
                {gardens.length}/{terrain.number_of_gardens} jardins créés
              </Badge>
